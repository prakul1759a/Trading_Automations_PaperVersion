...
class EnhancedCandleGUI:
    ...
    def match_date_key(self, date_data, target_date):
        """Handle possible date format variations including datetime.date objects"""
        ...
        for key in date_data.keys():
            # Check if key is a datetime or date object (NEW format)
            if isinstance(key, datetime):
                key_as_date = key.date()
            ...
            if key_as_date and target_date_obj:
                if key_as_date == target_date_obj:
                    return key

            # Check string variants (OLD format - backward compatible)
            if isinstance(key, str) and key in target_variants:
                return key
    ...
    def load_and_process_data(self):
        ...
        try:
            ...
            for indicator_key, date_data in data.items():
                ...
                if not df.empty:
                    # Calculate VWAP from complete candles
                    vwap_values = self.calculate_vwap_from_candles(candles)

                    # Calculate EMA from VWAP values
                    ema_values = self.calculate_ema_from_values(vwap_values)

                    # Add VWAP and EMA to dataframe (pad to match df length)
                    df['vwap'] = [None] * len(df)
                    df['ema'] = [None] * len(df)

                    # Map VWAP/EMA values to complete candles
                    vwap_idx = 0
                    for idx, row in df.iterrows():
                        if row['complete'] and vwap_idx < len(vwap_values):
                            df.at[idx, 'vwap'] = vwap_values[vwap_idx]
                            if vwap_idx < len(ema_values):
                                df.at[idx, 'ema'] = ema_values[vwap_idx]
                            vwap_idx += 1

                    # ensure chronological for signal calculation
                    df.sort_values('timestamp', inplace=True)

                    # Calculate signals: Entry when VWAP < EMA, Exit when VWAP >= EMA
                    sig = []
                    for _, row in df.iterrows():
                        ...
                        # Only generate signal on complete candles with valid VWAP/EMA
                        if not complete or pd.isna(vwap) or pd.isna(ema):
                            sig.append(0)
                            continue

                        # Entry when VWAP < EMA (oversold), Exit when VWAP >= EMA
                        if vwap < ema:
                            sig.append(1)  # Entry signal
                        else:  # vwap >= ema
                            sig.append(0)  # Exit signal
    ...
    def calculate_vwap_from_candles(self, candles):
        ...
        for ts in sorted(candles.keys()):
            ...
            volume = max(0, candle['volume'])  # Prevent negative volume
            cumulative_pv += typical_price * volume
            cumulative_vol += volume

            if cumulative_vol > 0:  # Only add VWAP when valid
                vwap = cumulative_pv / cumulative_vol
                vwap_values.append(vwap)

        return vwap_values

    def calculate_ema_from_values(self, values, period=24):
        ...
        # Use average of first 3 values as seed
        ema = sum(values[:3]) / 3
    ...
    def create_new_tab(self, indicator_key):
        ...
        columns = ('timestamp', 'open', 'high', 'low', 'close', 'volume', 'complete', 'vwap', 'ema', 'signal')
    ...
    def update_treeview(self, indicator_key, df):
        ...
        for _, row in df.iterrows():
            values = (
                ...
                row['volume'],
                row['complete'],
                f"{row['vwap']:.2f}" if not pd.isna(row['vwap']) else "N/A",
                f"{row['ema']:.2f}" if not pd.isna(row['ema']) else "N/A",
                row['signal']
            )
        ...
        tree.tag_configure("entry", background="#e6ffe6")  # Light green for entry signal (VWAP < EMA)
        tree.tag_configure("exit", background="#ffe6e6")   # Light red for exit signal (VWAP >= EMA)
