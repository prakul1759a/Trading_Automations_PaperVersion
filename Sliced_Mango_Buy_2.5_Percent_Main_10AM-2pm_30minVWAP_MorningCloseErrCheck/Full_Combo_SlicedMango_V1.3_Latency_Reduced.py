...
MAX_SLOTS            = 5    # Maximum slot entries per level per period
...
SLOT_ENTRY_PCTS_HIGH  = [0.02, 0.03, 0.06, 0.09, 0.12]
# Target: how far above entry price each slot exits as profit (slot 1..MAX_SLOTS)
SLOT_TARGET_PCTS_HIGH = [0.015, 0.02, 0.02, 0.02, 0.02]
...
ORDERS_CSV_PATH = r"D:\PrakulEditDaily\Paper_All_Versions\Working_Programs_Using_MongoQuoteMode\Sliced_Mango_Buy_2.5_Percent_Main_10AM-2pm_30minVWAP_MorningCloseErrCheck\orders.csv"
        ...
        try:
            ...
            if isinstance(loaded, dict):
                if '_volume_cache_' in loaded:
                    ...
                    vol_cache = loaded['_volume_cache_']
                else:
                    candles   = loaded
                    vol_cache = {}
                # Discard stale cache from crashed/incomplete sessions.
                # If fewer than MIN_CANDLES_REQUIRED complete candles exist for any
                # indicator the volume cache's last_vol values may be ahead of the
                # current DB's vol_in_day (data-collection program may have restarted
                # and reset its counters), causing incr_vol=0 for every candle and
                # breaking VWAP for the entire day.  Starting fresh resets last_vol=0
                # so incr_vol = cum_vol (always positive).
                max_complete = max(
                    (sum(1 for c in tfs.get('1min', {}).values() if c.get('complete', False))
                     for ik, dates in candles.items()
                     for dt, tfs in dates.items()),
                    default=0,
                )
                if max_complete < MIN_CANDLES_REQUIRED:
                    logging.warning(
                        f"Candle cache discarded: only {max_complete} complete candle(s) per indicator "
                        f"(need >= {MIN_CANDLES_REQUIRED}). Volume cache reset to avoid stale incr_vol=0."
                    )
                    strike_pair_candles = {}
                    return {}
                strike_pair_candles = candles
                return vol_cache
...
def get_market_timings():
    ...
    return {
        ...
        'last_trade_entry': now.replace(hour=14, minute=40, second=0,  microsecond=0),
        'force_exit_time':  now.replace(hour=14, minute=50, second=0,  microsecond=0),
        'kill_time':        now.replace(hour=15, minute=00, second=30, microsecond=0),
    }
