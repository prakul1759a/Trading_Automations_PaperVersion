...
PERIOD_MINUTES       = 5   # Length of each trading period in minutes
MAX_SLOTS            = 2    # Maximum slot entries per level per period
MIN_CANDLES_REQUIRED = 4   # Minimum complete candles before VWAP is used for entries
...
BASKET_TARGET_POINTS = 2000   # Total P&L >= this -> close all slots for this level
...
ORDERS_CSV_PATH = r"D:\PrakulEditDaily\Paper_All_Versions\Working_Programs_Using_MongoQuoteMode\Sliced_Mango_Buy_OI_3_and__Pecent_Main_10AM-2pm_5minVWAP\orders.csv"
...
def get_market_timings():
    ...
    return {
        ...
        'last_trade_entry': now.replace(hour=13, minute=50, second=0,  microsecond=0),
        'force_exit_time':  now.replace(hour=14, minute=00, second=0,  microsecond=0),
        'kill_time':        now.replace(hour=14, minute=1, second=30, microsecond=0),
    }
